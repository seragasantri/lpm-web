...
import { BookText, Plus, Trash2, Edit2, Save, RotateCcw, CalendarDays } from 'lucide-react';
import Textarea from '../../components/Textarea';
import FileUpload from '../../components/FileUpload';
import { getGpmf, updateGpmf } from '../../../lib/mockData';
import type { Gpmf, GpmfTugas, GpmfJadwal } from '../../../lib/types';
...
export default function GpmfPage() {
  useEffect(() => { document.title = 'Manajemen GPMF :: LPM Admin'; }, []);

  const [mode, setMode] = useState<'view' | 'edit'>('view');
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [originalData, setOriginalData] = useState<Gpmf | null>(null);
  const [form, setForm] = useState<Gpmf | null>(null);
  ...
  function updateTugas(index: number, field: keyof GpmfTugas, value: string) {
    if (!form) return;
    setForm(prev => prev ? {
      ...prev,
  ...
  function addJadwal() {
    ...
    setForm(prev => prev ? { ...prev, jadwal: [...prev.jadwal, newJadwal] } : null);
  }

  function removeJadwal(index: number) {
    if (!form) return;
    setForm(prev => prev ? { ...prev, jadwal: prev.jadwal.filter((_, i) => i !== index) } : null);
  }

  function updateJadwal(index: number, field: keyof GpmfJadwal, value: string) {
    if (!form) return;
    setForm(prev => prev ? {
      ...prev,
      jadwal: prev.jadwal.map((j, i) => i === index ? { ...j, [field]: value } : j),
    } : null);
  }
    ...
    <div>
      ...
      <div className="bg-gradient-to-r from-sky-600 to-sky-700 text-white py-8 px-6 mb-6 rounded-b-2xl shadow-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <BookText size={24} />
            <div>
              <h1 className="text-2xl font-bold">Manajemen GPMF</h1>
              <p className="text-sky-100 text-sm mt-0.5">Gugus Penjaminan Mutu Fakultas</p>
            </div>
          </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <h2 className="text-base font-bold text-slate-700 mb-4">Tentang GPMF</h2>
          {mode === 'view' ? (
            ...
            <Textarea
              ...
              placeholder="Masukkan deskripsi tentang GPMF..."
              rows={4}
            />
          )}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-bold text-slate-700">Tugas & Fungsi GPMF</h2>
            {mode === 'edit' && (
              <button
                onClick={addTugas}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-slate-700">Jadwal Kegiatan</h2>
            </div>
            {mode === 'edit' && (
              <button
                onClick={addJadwal}
          ...
          <div className="space-y-3">
            {form.jadwal.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-6">Belum ada jadwal kegiatan.</p>
            ) : (
              form.jadwal.map((jadwal, index) => (
                <div key={jadwal.id} className="border border-slate-200 rounded-xl p-4 bg-slate-50/50">
                  {mode === 'view' ? (
                    <div className="grid grid-cols-4 gap-4 text-sm">
                      <div>
                        <span className="text-xs font-semibold text-slate-500 block">Hari</span>
                        <span className="text-slate-700 font-medium">{jadwal.hari}</span>
                      </div>
                      <div className="col-span-2">
                        <span className="text-xs font-semibold text-slate-500 block">Kegiatan</span>
                        <span className="text-slate-700">{jadwal.kegiatan}</span>
                      </div>
                      <div>
                        <span className="text-xs font-semibold text-slate-500 block">Waktu & Lokasi</span>
                        <span className="text-slate-700">{jadwal.waktu}</span>
                        <span className="text-slate-400 text-xs block">{jadwal.lokasi}</span>
                      </div>
                    </div>
                  ) : (
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-semibold text-slate-500">Jadwal #{index + 1}</span>
                        <button
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                        ...
                        <div className="md:col-span-2">
                          ...
                          <input
                            ...
                            onChange={(e) => updateJadwal(index, 'kegiatan', e.target.value)}
                            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-400"
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value={jadwal.waktu}
                            onChange={(e) => updateJadwal(index, 'waktu', e.target.value)}
                            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-400"
                      ...
                      <div className="mt-2">
                        ...
                        <input
                          type="text"
                          value={jadwal.lokasi}
                          onChange={(e) => updateJadwal(index, 'lokasi', e.target.value)}
                          className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-400"
