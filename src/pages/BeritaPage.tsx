...
import { getBerita, getKategori } from '../lib/mockData';
import type { Berita, Kategori } from '../lib/types';
...
const kategoriColors: Record<string, string> = {
  ...
  ISO: 'bg-sky-100 text-sky-700 border-sky-200',
...
export default function BeritaPage() {
  ...
  useEffect(() => {
    Promise.all([getBerita(), getKategori()]).then(([beritaData, kategoriData]) => {
      setBerita(beritaData.filter((b: Berita) => b.status === 'published'));
  ...
  const filtered = berita.filter(b => {
    const matchKategori = !selectedKategori || b.kategori === selectedKategori;
              ...
              >
                {item.gambar ? (
                  <div className="h-48 overflow-hidden relative">
                    ...
                    />
                    <div className={`absolute top-4 left-4 px-3 py-1.5 rounded-md text-xs font-extrabold shadow-md ${kategoriColors[item.kategori] || 'bg-slate-100 text-slate-600'}`}>
                      {item.kategori}
                    </div>
                  </div>
                ) : (
